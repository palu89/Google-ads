...
### Added
- 新增 `references/persona-expression-variance.md`，用于统一管理去 AI、人类纹理、生活锚点、轻量可见瑕疵边界与 active persona 的表达纹理簇映射。
- 新增 `references/story-structure-formula-library.md`，用于沉淀标题、开头、转折、收尾、人物设定与情绪推进的结构公式，而不再只停留在对话分析里。
- 新增 `references/story-mode-switch-sop.md`，用于把 `高表现版` 结构稿系统性收口为 `合规发布版`，避免只靠人工临场改写。

### Changed
- 更新 `SKILL.md`，把 `persona-expression-variance.md` 正式接入人设系统维护与可选内容模板包路由，并补充“允许有人类纹理但不机械造人味”的总原则。
- 更新 `SKILL.md`，把 `story-structure-formula-library.md` 接入可选内容模板包、输出约束与资源导航，明确高张力钩子和故事型推进要先选结构公式。
- 更新 `references/content-output-enforcement.md`，新增“表达纹理计划”与“去 AI / 人类纹理复核”，并把 `纹理簇` 与 `生活锚点` 纳入最小执行记录。
- 更新 `references/content-output-enforcement.md`，新增“结构公式计划”与“结构公式复核”，并把标题 / 开头 / 转折 / 收尾公式纳入最小执行记录。
- 更新 `references/p0-persona-branding.md`，把人设包装改为强制回答“情绪底色 / 生活切入口 / 句子粗糙度 / 口头纹理”四项，并补充去 AI 总规则。
- 更新 `references/persona-governance.md`，新增 `emotional_base`、`life_anchors`、`spoken_tics`、`texture_rules` 作为建议补齐的人设表达字段，并明确其为 persona-local。
- 更新 `references/persona-registry.md`，扩展人设差异化强制检查与字段模板，加入情绪底色、生活锚点、口头纹理与纹理规则占位。
- 为第一批高频 persona：`old-cat-taiwan-stocks`、`laowang-taiwan-rebuild`、`stock-analyst`、`taiwan-stock-big-brother`、`xinjie-single-mom-survival` 回填 `emotional_base`、`life_anchors`、`spoken_tics`、`texture_rules`，把去 AI 和细微差异真正落到高频 persona 本体。
- 为第二批高频 persona：`taiwan-stock-captain`、`laoqiu-88`、`retired-ajie-taiwan-veteran`、`laozhou-kline-veteran`、`not-a-stock-god` 回填 `emotional_base`、`life_anchors`、`spoken_tics`、`texture_rules`，继续把高频 persona 从“共享纹理簇”推进到“具名表达边界”。
- 更新 `references/usage-scope.md`，将 `persona-expression-variance.md` 纳入可选内容模板包。
- 更新 `references/content-performance.md`，新增“结构张力”要求，明确故事型 / 高张力内容必须有清晰的标题、开头、转折与收尾功能分工。
- 更新 `references/story-structure-formula-library.md`，正式拆分 `高表现版` 与 `合规发布版` 两层；默认正式输出走 `合规发布版`，高张力演练仅用于拆解、测试或二次收口前的结构草稿。
- 更新 `references/story-structure-formula-library.md`，把 `story-mode-switch-sop.md` 接回双模式链路，明确高表现稿转可发布稿时优先走独立 SOP。
- 更新 `references/content-output-enforcement.md`，要求在调用结构公式时明确选择 `高表现版` 或 `合规发布版`，并把模式记录进最小执行记录。
- 更新 `references/content-output-enforcement.md`，要求高表现稿若要交付可发布版，必须再走一次 `story-mode-switch-sop.md`。
- 收敛 `P5` 路由为单一结构源，清理多入口残留，避免运行时继续漂移。
- 更新 `references/p5-evening-lessons-high-performance.md`，把 `P5` 统一为唯一结构源，并新增对“抽象术语先行”和“编号式自助收尾”的禁回退约束。
- 更新 `references/content-output-enforcement.md`，把 `P5 + 高表现版` 下的“抽象术语先行”和“编号式自助收尾”明确列为直接重写条件。
- 更新 `references/p5-evening-lessons-high-performance.md`，补入默认正文区间、标题公式占位、互动收尾公式占位，以及合格 / 不合格骨架示例，使模板本体更接近可直接复用。
- 更新 `references/content-output-enforcement.md`，为 `P5 + 高表现版` 增加默认字数区间与超长时的 `REVISE` 复核条件，避免新模板只停留在说明层。
- 更新 `references/content-performance.md`，明确可发布内容的结构张力应主要来自痛点、判断、提醒和关系感，而不是内幕感或收益刺激。
- 更新 `SKILL.md`、`references/usage-scope.md`、`references/skill-validation.md` 与 `scripts/validate_platform_operations.py`，把 `story-mode-switch-sop.md` 纳入正式路由、可选内容模板包与校验链。
- 更新 `references/usage-scope.md`，将 `story-structure-formula-library.md` 纳入可选内容模板包。
- 更新 `references/skill-validation.md` 与 `scripts/validate_platform_operations.py`，把 `persona-expression-variance.md`、`story-structure-formula-library.md` 及其接线状态纳入校验。
- 更新 `references/skill-validation.md` 与 `scripts/validate_platform_operations.py`，新增对 `高表现版 / 合规发布版` 双模式存在性的校验。
- 进一步细化 `references/p0-persona-branding.md` 中 `馨姊｜單親媽媽的台股生存戰` 与 `台股大師兄` 的纹理边界：前者允许轻量停顿与生活化毛边，后者以沉静停顿和非对称句子去 AI，不依赖显性错字。

### Impact
- 后续在同一内容模板下切换 persona 时，不再只靠换名字和头衔，而必须同时调整情绪底色、生活锚点、句子粗糙度和口头纹理。
- “错别字、错标点、空格代标点、口癖” 现在有了明确上限与适用范围，不再允许泛化成所有 persona 的默认写法。
- 稳重理性型 persona 现在默认优先使用节奏变化和轻停顿去 AI；生活压力型 persona 才允许在不影响理解的前提下轻量使用可见瑕疵。
- 当前已完成两批共 `10` 个高频 `active` persona 的表达字段回填，不再只依赖纹理簇默认值，而是具备各自的情绪底色、生活锚点、口头纹理和句子毛边边界。
- 标题、开头、转折、收尾的公式现在已进入正式规则层；后续需要高张力、故事型或反差钩子的内容，不再只靠口头说明，而有独立 reference 可复用、可审稿、可校验。
- 故事结构公式现在不再只有单一版本：内部演练可用 `高表现版` 拉出张力，但只要进入正式 Threads 输出，默认必须收口到 `合规发布版`。
- 现在 `高表现版 -> 合规发布版` 已有独立转换 SOP；后续如果先拉高张力再收口，不再需要临时解释“该怎么改”，而是有正式步骤、校验点和最小执行记录。
- `P5` 现在仅保留单一结构源；若 TG 仍写出不符合当前模式的内容，问题只能继续往生成层和监督层查，而不再是路由残留。
- `P5 + 高表现版` 现在明确不鼓励以抽象术语先行，也不鼓励再收成编号式自助稿。
- `P5 + 高表现版` 模板现在已补齐标题、互动、长度和示例骨架；后续查看单文件模板时，不必再额外口头解释这些缺失项。
- 这是高影响规则升级：会直接改变 P0 及后续内容模板对“像真人说话”的判断标准。
...
- 按用户“添加的人设可以正常使用”的要求，将上述 `15` 个批量补建 persona 从 `draft` 切换为 `active`，并同步补充高优先级识别信号与当前 `active` 人设清单。
- 更新 `taiwan-stock-big-brother` 与 `xinjie-single-mom-survival` 的 persona 字段与 `references/p0-persona-branding.md`，补入去 AI、生活化、情绪背景与共鸣式互动的专属模板约束。
...
- 当前 `active` persona 池已由 `10` 个扩展到 `25` 个；Telegram 侧读取 `active` 人设时，将能看到这批新增 persona 并正常调用。
- 本轮严格遵守“重复的不更新、没有的进行添加”的边界：旧有重复条目未改动，只把上轮新增但不可用的 `15` 个 persona 提升为可执行状态。
- `台股大師兄` 现在更明确走“收盤後前輩式慢聊拆盤”路线，不再容易滑回教科书或研报口吻。
- `馨姊｜單親媽媽的台股生存戰` 现在更明确走“生活缝隙里的复盘与情绪修正”路线，不再容易被写成通用励志或标准教学稿。
...
- 更新 `SKILL.md`、`references/p0-persona-branding.md`、`references/p1-trading-psychology.md`、`references/p2-morning-brief.md`、`references/p3-intraday-tracker.md`、`references/p4-postmarket-recap.md`、`references/p5` 晚间教育结构源、`references/macro-news-synthesizer.md` 与 `references/single-stock-deepdive.md`，要求内容输出先读 `content-output-enforcement.md`。
...
- 审计发现 `p1-trading-psychology.md`、`p2-morning-brief.md`、`p3-intraday-tracker.md`、`p4-postmarket-recap.md`、`P5` 晚间教育结构源、`macro-news-synthesizer.md` 与 `single-stock-deepdive.md` 目前主要停留在 `SKILL.md` 声明层，未进入近期真实维护主链路。
...
- 预期结果：`pass`
# 2026-03-22
- Consolidated `P5` onto a single live structure source and removed stale routing references.
- Unified `P5` routing onto `references/p5-evening-lessons-high-performance.md` as the only structure source.
- `P5` 合规发布版 now uses the same structure source and must be收口 through `references/story-mode-switch-sop.md`.
