...
本项目以 Codex 系统层 skills 为唯一系统能力主源。
...
- 平台账户运营项目不复制系统层 skill 本体
- 搜索、摘要、学习能力用于平台规则核查、素材吸收和持续优化
- 浏览器自动化和外部集成属于运行端能力，必须通过 OpenClaw 项目封装后再实际调用
...
- 用户要设计或运营 Threads 多账号矩阵，例如 `10` 个账户的发帖、钩子、回复、涨粉和组合复盘。
...
- 允许有人类纹理，但不允许机械化造“人味”；具体做法以 `references/persona-expression-variance.md` 为准。
- 若任务明确需要高张力标题、故事感推进、反差钩子或结构公式演练，具体做法以 `references/story-structure-formula-library.md` 为准。
- 若已先写出高张力样稿、需要转成 Threads 可发布版，具体做法以 `references/story-mode-switch-sop.md` 为准。
- 先讲逻辑，再讲观点；先讲条件，再讲价位。
...
- 若进入内容生产或模板验证，再额外明确：`情绪底色`、`生活锚点`、`句子粗糙度`、`口头纹理`
...
- 多账号运营属于显式增强链路，只在用户明确进入账号矩阵、组合增长或统一执行编排时启用
- 多账号运营先做组合治理、账号基线、钩子治理和数据回收，不直接进入 `10` 账户全自动并发
...
2. 人设系统维护：新增、修改、停用、占位符治理、字段治理、自动匹配门槛调整时，读取 `references/usage-scope.md`、`references/persona-governance.md`、`references/persona-registry.md`、`references/persona-expression-variance.md`、`references/skill-validation.md`、`CHANGELOG.md`；如涉及可发布性或高风险叙事，再补读 `references/threads-account-compliance.md` 与 `references/threads-copy-compliance.md`
...
4. 内容运营强化：当用户明确要开始或强化 Threads 平台账户内容运营时，先读取 `references/usage-scope.md`、`references/threads-content-operations-workflow.md`、`references/persona-governance.md`、`references/persona-registry.md`、`references/threads-review-workflow.md`、`references/threads-copy-compliance.md`；如涉及账号资料、Bio、实名背书、外链，再补读 `references/threads-account-compliance.md`
   ...
   - 若明确进入 `3` 个以上账号、账号矩阵、统一发帖/回复/增长任务，优先补读 `references/threads-portfolio-operations-architecture.md`
   - 若需要把多账号能力落成项目层运行封装，优先补读 `references/threads-multi-account-skills-blueprint.md`
   - 若需要给多账号运营开第一批落地任务，优先补读 `references/threads-multi-account-batch-01-plan.md`
   - 若需要正式填写 `10` 账户矩阵总表，优先补读 `references/threads-account-matrix-master-board.md`
   - 若需要决定 `2-3` 个优先启动账号，优先补读 `references/threads-priority-account-activation-framework.md`
   - 若需要统一管理多账号钩子与开头去重，优先补读 `references/threads-shared-hook-library-framework.md`
   - 若需要定义发帖输入、输出与失败回写字段，优先补读 `references/threads-publish-writeback-spec.md`
   - 若需要定义 insights 抓取、回写和组合比较字段，优先补读 `references/threads-insights-collection-spec.md`
...
5. 可选内容模板包：只有在用户明确进入内容运营对话、要求测试模板或验证样稿时，才读取 `references/threads-content-operations-workflow.md`、`references/p0-persona-branding.md`、`references/persona-expression-variance.md`、`references/tone-style-tw.md`、`references/content-performance.md`、`references/content-dedup-audit.md`、`references/content-continuity.md`、`references/p1-trading-psychology.md`、`references/p2-morning-brief.md`、`references/p3-intraday-tracker.md`、`references/p4-postmarket-recap.md`、`references/p5-evening-lessons-high-performance.md`、`references/macro-news-synthesizer.md`、`references/single-stock-deepdive.md`
   - 在进入任何内容模板前，先读 `references/content-output-enforcement.md`
   - 若用户明确要高张力钩子、故事型推进或结构公式演练，再补读 `references/story-structure-formula-library.md`
   - 若用户明确要把高表现稿收成可发布稿，再补读 `references/story-mode-switch-sop.md`
   - 若模块是 `P5`，统一读取 `references/p5-evening-lessons-high-performance.md`
   - 若模块是 `P5` 且模式是 `合规发布版`，再补读 `references/story-mode-switch-sop.md`
...
- 若内容明确使用故事型 / 高张力结构，必须先选定标题、开头、转折、收尾公式，不能只凭语气词硬拉情绪
- 涉及账号合规或文案审核时，输出必须区分：
  - `违规高风险`：可能触发移除、限制、封禁
...
- `references/persona-expression-variance.md`：去 AI、人类纹理、生活锚点与 persona 微差异表达规则
- `references/story-structure-formula-library.md`：标题、开头、转折、收尾、人物设定与情绪推进的结构公式库
- `references/story-mode-switch-sop.md`：把高表现版结构稿收口为合规发布版的正式转换 SOP
...
- `references/threads-portfolio-operations-architecture.md`：Threads 多账号矩阵的层级架构、账户分工与 Codex/OpenClaw/TG 分层
- `references/threads-multi-account-skills-blueprint.md`：Threads 多账号运营所需项目层 skills 蓝图与分工
- `references/threads-multi-account-batch-01-plan.md`：Threads 多账号运营第一批落地计划、交付物与验收标准
- `references/threads-account-matrix-master-board.md`：Threads `10` 账户矩阵总表模板，用于定义账户类型、角色、主副线与启动状态
- `references/threads-priority-account-activation-framework.md`：优先启动账户框架，用于决定首批 `2-3` 个重点运营账号
- `references/threads-shared-hook-library-framework.md`：共享钩子库框架，用于统一多账号钩子类型、风险分级与去重
- `references/threads-publish-writeback-spec.md`：发帖与状态回写规格，用于定义发布输入、输出、失败与审稿前置字段
- `references/threads-insights-collection-spec.md`：insights 回收规格，用于定义账号、帖子和组合层指标回写方式
...
- `references/p5-evening-lessons-high-performance.md`：可选内容模板包，`P5` 唯一结构模板；高表现版起稿、合规发布版经 `story-mode-switch-sop.md` 收口
