...
- 删除 `references/p5-evening-lessons.md`，取消旧 `P5` 课堂模板文件，避免运行时继续回退到旧骨架。
- 更新 `references/p5-evening-lessons-high-performance.md`，把 `P5` 统一为唯一结构源，并新增对“抽象心理学标签起手”和“三问式自助清单收尾”的禁回退约束。
- 更新 `references/content-output-enforcement.md`，把 `P5 + 高表现版` 下的“抽象术语先行”和“三问收尾”明确列为直接重写条件。
...
- `P5` 现在已不再存在“旧模板文件可回退”的情况；若 TG 仍写出旧味道，问题只能继续往生成层和监督层查，而不再是模板路由残留。
- `P5 + 高表现版` 现在明确不鼓励以 `确认偏误 / 信息过载 / 情绪复位` 等术语先行，也不鼓励再收成“今晚问自己三个问题”的自助稿。
...
- 预期结果：`pass`
# 2026-03-22
- Deleted the legacy `references/p5-evening-lessons.md` template file from the live skill.
