...
- 为 `old-cat-taiwan-stocks`、`laowang-taiwan-rebuild`、`stock-analyst`、`taiwan-stock-big-brother`、`xinjie-single-mom-survival` 回填 `emotional_base`、`life_anchors`、`spoken_tics`、`texture_rules`，把去 AI 和细微差异真正落到高频 persona 本体。
...
- 这 5 个高频 `active` persona 现在已经不只依赖纹理簇默认值，而是具备各自的情绪底色、生活锚点、口头纹理和句子毛边边界。
