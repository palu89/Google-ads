...
当前 `active` 人设：`taiwan-stock-captain`、`old-cat-taiwan-stocks`、`laoqiu-88`、`retired-ajie-taiwan-veteran`、`laozhou-kline-veteran`、`comeback-trader-15y`、`master-di-short-swing`、`laowang-taiwan-rebuild`、`stock-analyst`、`lige-data-driven-veteran`
...
- status: draft
- profile_type: composite
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有短线、板块轮动类人设的重叠度与可执行边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；当前按待复核草案管理，不直接参与自动匹配
- display_name: 飙股逻辑师
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有长线、避坑与陪伴型人设的重叠度复核后再决定是否激活
- source_note: 依据用户提供描述建档；“免费回答粉丝个股问题”等承诺型表述当前不作为默认可发布规则
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 含“單親媽媽”“資產近億”等强个人经历与财富结果叙事，当前仅有用户提供设定，未核对前不进入自动匹配或对外成稿
- source_note: 依据用户提供描述建档；当前按高风险个人逆转叙事草案管理，不将“資產近億”作为默认可发布背书
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 含“從證交所退休”这类机构背景背书，当前仅有用户提供设定，未核对前不进入自动匹配或对外成稿
- source_note: 依据用户提供描述建档；当前按高风险机构背书草案管理，不把“證交所退休”作为已核实名背书对外使用
- display_name: 台股大師兄
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有退休、茶馆与长线心法类人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；“交易所不敢寫進教科書”属高刺激表达，不作为默认可发布口径
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与财报、基本面与女性陪伴型人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；当前按待复核草案管理，不直接参与自动匹配
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 含單親、負債與財務獨立等强个人逆转叙事，当前仅有用户提供设定，未核对前不进入自动匹配或对外成稿
- source_note: 依据用户提供描述建档；当前按高风险逆境叙事草案管理，不把“財富獨立”作为默认可发布背书
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有实战派、纪律派与成长叙事类人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；“資產一路做大”“長期贏”属高刺激结果表述，不作为默认可发布背书
...
- status: draft
- profile_type: composite
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有导航、船长和市场生存类人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；当前按待复核草案管理，不直接参与自动匹配
- display_name: 市場導航員
...
- status: draft
- profile_type: composite
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有主力、短线与强势股类人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；当前按待复核草案管理，不直接参与自动匹配
- display_name: 黑馬捕手
...
- status: draft
- profile_type: composite
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与选股、规律与知识入口类人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；当前按待复核草案管理，不直接参与自动匹配
- display_name: 選股獵人
...
- status: draft
- profile_type: composite
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有过来人、方法论与心理型人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；当前按待复核草案管理，不直接参与自动匹配
- display_name: 我不是股神
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 含“一夜暴富”“單車變寶馬”等高刺激财富叙事，当前仅有用户提供设定，未核对前不进入自动匹配或对外成稿
- source_note: 依据用户提供描述建档；当前按高风险市场见闻叙事草案管理，不将极端暴富或家破人亡表述作为默认可发布口径
- display_name: 响股不用锤
...
- status: draft
- profile_type: composite
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有全球资金视角、顺势与强势题材类人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；“帶你一起飛、改變不止人生”属高刺激表达，不作为默认可发布口径
- display_name: 台股防空蛋
...
- profile_type: fictionalized
- verification_state: unverified
- status_reason: 依据用户提供清单批量建档；当前先设为 `draft`，待与现有规律、方法论与长线心法类人设的边界复核后再决定是否激活
- source_note: 依据用户提供描述建档；当前按待复核草案管理，不直接参与自动匹配
