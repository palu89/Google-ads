...
- 旧预置执行人设已清空，当前启用中的 `active` personas 为 `taiwan-stock-captain`、`old-cat-taiwan-stocks`、`laoqiu-88`、`retired-ajie-taiwan-veteran`、`laozhou-kline-veteran` 与 `comeback-trader-15y`。
- 在用户未明确指定其他 persona 时，默认仍以 `taiwan-stock-captain` 作为兜底执行人设。
- 当前 `active` personas 已扩展为 `taiwan-stock-captain`、`old-cat-taiwan-stocks`、`laoqiu-88`、`retired-ajie-taiwan-veteran`、`laozhou-kline-veteran` 与 `comeback-trader-15y`；其中“老貓 / 分點 / 主力 / 洗盤 / 盤後數據”相关输入会优先命中 `old-cat-taiwan-stocks`。
