...
当前 `active` 人设：`taiwan-stock-captain`、`old-cat-taiwan-stocks`、`laoqiu-88`、`retired-ajie-taiwan-veteran`、`laozhou-kline-veteran`、`comeback-trader-15y`、`master-di-short-swing`、`laowang-taiwan-rebuild`
...
### 9. amei-exchange-retirement-journal
- status: draft
...
### 10. stock-analyst-placeholder
- status: draft
- profile_type: placeholder
- verification_state: unverified
- status_reason: 当前仅收到通用名称“股票分析师”，缺少差异化定位、受众、语气、主题与禁区，先按占位人设建档
- placeholder_fields: role_label,signature_line,life_stage,authority_signals,target_audience,core_topics,tone,hook_pain_points,default_cta,keywords
- source_note: 依据用户提供的通用人设名称建档；在补齐运营字段前，不参与自动匹配，也不作为可发布人设使用
- display_name: 股票分析师
- role_label: {{role_label}}
- signature_line: {{signature_line}}
- life_stage: {{life_stage}}
...
- hook_pain_points: {{hook_pain_point}}
- default_cta: {{default_cta}}
- avoid: 编造履历、泛化成任何人都能套用的分析师模板、用未核实头衔建立权威
- keywords: {{market_focus}}
