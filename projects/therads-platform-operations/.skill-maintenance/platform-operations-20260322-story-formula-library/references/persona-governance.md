...
若当前存在 `2` 个以上 `active` persona，或准备进入多 persona 试运行，再补读 `multi-persona-coexistence-governance.md`。
...
- 人设字段默认只对该 `persona_id` 本身生效，不会自动继承到其他人设
- `output_limit`、`tone`、`signature_line`、`default_cta`、`avoid`、`keywords` 这类 persona-local 字段，只能作为该人设自己的约束
- `emotional_base`、`life_anchors`、`spoken_tics`、`texture_rules` 这类表达纹理字段，也属于 persona-local，只能在对应 persona 或对应纹理簇下使用
- 若某个限制需要全局生效，必须单独写入共享 reference，并明确标注适用范围
- 不要把单一 persona 的输出限制，误写成 `SKILL.md` 或共用模板的默认规则
...
4. 若该 persona 预期要长期做内容，建议同时补 `emotional_base`、`life_anchors`、`spoken_tics` 或 `texture_rules`
5. 满足激活条件后再改为 `active`
